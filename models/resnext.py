...
class CifarResNeXt_new(nn.Module):
    """
    ResNext optimized for the Cifar dataset, as specified in
    https://arxiv.org/pdf/1611.05431.pdf
    """
    def __init__(self, block, depth, cardinality, base_width, num_classes,
                 dropout):
        super(CifarResNeXt_new, self).__init__()
        self.num_classes = num_classes
        #Model type specifies number of layers for CIFAR-10 and CIFAR-100 model
        assert (depth -
                2) % 9 == 0, 'depth should be one of 29, 38, 47, 56, 101'
...
def resnext29_4_24_new(num_classes=10, dropout=False, stride=None):
    """Constructs a ResNeXt-29, 4*24d model"""
    model = CifarResNeXt_new(ResNeXtBottleneck, 29, 4, 24, num_classes, dropout)
    return model
